...
const Post = ({ data, postID }: PostData) => {
  ...
  if (!data) {
    return <div className="text-2xl">This post does not exist</div>;
  }

  return (
    <div className="bg-gray-200 w-full rounded-2xl p-4 mb-2">
      ...
      {clickEdit && <Update open={clickEdit} setOpen={setCE} postID={postID} />}

      {/*Modal for Delete Option*/}
      {clickDelete && <Delete open={clickDelete} setOpen={setCD} postID={postID} />}
      ...
      <div className="mb-2 text-sm font-medium">
        Tags:
        {data.tags?.map((tag) => (
          <span key={tag} className="ml-2 bg-blue-500 text-xs text-white rounded-xl px-2 py-1">
            {tag}
          </span>
        ))}
      </div>
      <div className="text-md">
        <LikePost idPost={postID} is_liked={data?.is_liked} like_count={data?.like_count} />
      </div>
    </div>
  );
};
