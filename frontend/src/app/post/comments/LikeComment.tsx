...
const LikeButton = ({ postID, idComment, is_liked = false, like_count = 0 }: LikeComment) => {
  const [currLike, setCL] = useState(is_liked);
  const [likeCount, setLC] = useState(like_count);
  const [error, setError] = useState("");

  // Get the most recent likes and if the user currently liked the comment or not
  useEffect(() => {
    setLC(like_count);
    setCL(is_liked);
  }, [is_liked, like_count]);
  ...
  const clickLike = async () => {
    ...
    //Attempt to comment likes otherwise we get an error
    try {
      const response = await fetch(`http://localhost:5001/api/comments/${postID}/${idComment}`, {
        method: "POST",
        headers: {
      ...
      // Get the comment's like count and if the user has liked it or not
      setCL(data.is_liked);
